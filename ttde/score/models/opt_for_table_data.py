...
class PAsTTSqrOpt(PAsTTOptBase):
    ...
    def log_int_q2(self):
        """
        Exact log ∫ (∑_k q_k(x))^2 dx including cross-terms,
        slicing components properly from self.tt.value.
        """
        # 1) per-dimension D^(k): shape [d, m, m]
        Ds = jax.vmap(type(self.bases).l2_integral)(self.bases)

        n_comp = self.n_components
        # 2) meshgrid on component indices [0,1,…,n_comp-1]
        idx_i, idx_j = jnp.meshgrid(
            jnp.arange(n_comp),
            jnp.arange(n_comp),
            indexing='ij'
        )
        idx_i = idx_i.reshape(-1)  # (n_comp**2,)
        idx_j = idx_j.reshape(-1)

        def one_pair(i, j):
            # slice out the i-th and j-th TTOpt components
            tt_i = TTOpt(
                first = self.tt.value.first[i],
                inner = self.tt.value.inner[i],
                last  = self.tt.value.last[i],
            )
            tt_j = TTOpt(
                first = self.tt.value.first[j],
                inner = self.tt.value.inner[j],
                last  = self.tt.value.last[j],
            )

            # permute Ds according to component j
            D_op = TTOperatorOpt.rank_1_from_matrices(Ds[self.permutations[j]])
            # apply operator to tt_j
            mapped = normalized_dot_operator(tt_j, D_op)
            log_inner = normalized_inner_product(tt_i, mapped).log_norm

            # cross-term weight: 1 if i==j, else 2
            weight = jnp.where(i == j, 1.0, 2.0)
            return log_inner, weight

        # vectorize over all (i,j) pairs
        log_vals, weights = jax.vmap(one_pair)(idx_i, idx_j)

        # sum in linear space, then take log
        terms = weights * jnp.exp(log_vals)
        return jnp.log(jnp.sum(terms))
